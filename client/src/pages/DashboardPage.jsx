  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
      ...
      <section className="bg-white rounded-3xl p-6 sm:p-8 border border-slate-200 shadow-md">
        ...
        {/* Quick Place Shortcuts if any */}
        {favorites.places.length > 0 && (
          <div className="mt-5 pt-4 border-t border-slate-100 flex flex-wrap items-center gap-2">
            <span className="text-xs font-semibold text-slate-400">Quick Destination:</span>
            {favorites.places.map((place) => (
              <button
                key={place.id}
                type="button"
                onClick={() => handleQuickPlaceSelect(place.stop_id, true)}
                className="px-3 py-1 rounded-lg text-xs font-medium bg-slate-100 text-slate-700 hover:bg-blue-50 hover:text-blue-700 border border-slate-200 transition-colors flex items-center gap-1"
              >
